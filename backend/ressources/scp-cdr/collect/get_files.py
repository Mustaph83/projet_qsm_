...
def copy_files_from_remote(cfg):
    ...
    connection_type, src_host, src_port, src_username, src_password, ssh_key, src_dir, pattern_dir, date_var, formate, pattern, parse_dir= [*cfg['source'].values()]
    dest_dir = cfg['destination'].get('directory')  
    files_dest = os.listdir(dest_dir)
    parse_ser = os.listdir(parse_dir)
    data = "data"
    if date_var == 'today':
    ...
    if connection_type == 'ssh':
        ...
        for dir in rfiles:
            if os.path.isdir(dir):
                if bool(re.match(pattern_dir, dir)):
                    src_dir = os.path.join(src_dir, dir)
                    file_to_copy = os.listdir(src_dir)
                    for file in file_to_copy:
                        ...
                        if pattern != '':
                            if bool(re.match(pattern, file)):
                                ...
                                id_service = file.split('_')[2]
                                dirparse = os.path.join(parse_dir, id_service)
                                if os.path.exists(dirparse):
                                    shutil.copy(path_file, dirparse)
                                elif not os.path.exists(dirparse):
                                    os.mkdir(dirparse)
                                    shutil.copy(path_file, dirparse)
                                else:
                                    continue
